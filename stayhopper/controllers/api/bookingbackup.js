...
router.post("/", async (req, res) => {
  ...
  if (UB._id) {
    ...
    for (var i = 0; i < rooms.length; i++) {
      ...
      if(rooms[i].extraslot_cleaning){
        extraslot_cleaning = parseInt(rooms[i].extraslot_cleaning);
      }
            ...
            if (requested_slots[0] && requested_slots[0].slots.length > 0) {
              ...
              if (typeof requested_slots[1] == "undefined") {
                for (
                  ...
                  m > slots.length - 1 - extra_slots_number;
                  m--
                ) {
  ...
  curl
    .setBody({
      ...
      return_auth:
        "http://18.222.248.60/api/payment/success?booking_id=" + UB._id,
      return_can:
        "http://18.222.248.60/api/payment/failed?booking_id=" +
        UB._id +
        "&promocode=" +
        promocode,
      return_decl:
      ...
      if (is_exists.length <= 0) {
        ...
        curl
          .setBody({
            ...
            return_auth:
              "http://18.222.248.60/api/payment/success?booking_id=" + UB._id, //config.app_url+"/api/payment/success?booking_id="+UB._id,
            return_can:
              "http://18.222.248.60/api/payment/failed?booking_id=" + UB._id, //config.app_url+"/api/payment/failed?booking_id="+UB._id,
            return_decl:
              "http://18.222.248.60/api/payment/failed?booking_id=" + UB._id //config.app_url+"/api/payment/failed?booking_id="+UB._id
          })
          .post("https://secure.telr.com/gateway/order.json")
